...
class PersistentLookupCache : public BaseCache,
 ...
 public:
  ...
  bool add_and_persist(const K& k, const V& v) {
    ...
    // Wenn Index bereits geladen, aktualisiere die In-Memory-Struktur atomar (kopiere, erweitere,
    // store)
    ...
    if (oldmap) {
      ...
      std::atomic_store(&index_ptr_,
                        std::static_pointer_cast<const std::unordered_map<K, std::string>>(newmap));
    }
    return true;
  }
