...
class CacheManager {
 public:
  ...
  static std::shared_ptr<InMemoryCache<K, V>> create_inmemory_cache(int id);
  //   {
  ...
  template <typename K, typename V>
  std::shared_ptr<PersistentLookupCache<K, V>> create_persistent_cache(
      int id, CacheSerializer<K> key_ser, CacheDeserializer<K> key_des, CacheSerializer<V> val_ser,
  ...
  //     }
  //     auto c = std::make_shared<PersistentLookupCache<K, V>>(cache_name, key_ser, key_des,
  //     val_ser,
  //                                                            val_des, base_dir);
  //     map_.emplace(id, c);
  //     return c;
  ...
  template <typename K, typename V>
  std::shared_ptr<BaseCache> get_cache_base(int id);
  //    {
  //     std::lock_guard<std::mutex> g(mutex_);
  //     auto it = map_.find(id);
 ...
 private:
  ...
  static std::unordered_map<int, std::shared_ptr<BaseCache>> map_;
};
...
#include <sgpp/combigrid/miscellaneous/caching/cache_manager.tpp>