...
% shared_ptr(sgpp::combigrid::SGGenInstr) % shared_ptr(sgpp::combigrid::CompleteSGGenInstr) %
    ...
    // HyperCubeArea and Lvl2GPCntFunc in the signatures below; the aliases are not wrapped
    // themselves.
...
% include "Functions.i" %
    ...
    // genReducedMIVecWithCoeffs returns std::pair<LvlMIVec, std::vector<CTCoeffType>>. std_pair.i
    // cannot be instantiated for it: MIVec is not copy-assignable (const member), which the
    // generated 'first' / 'second' setters of a pair proxy require. The pair is returned as the
    // Python tuple (LvlMIVec, tuple of ints) instead, which is also what std_pair.i returns for a
    // pair.
    %
    typemap(out) std::pair<sgpp::combigrid::LvlMIVec, std::vector<sgpp::combigrid::CTCoeffType> > {
  const std::pair<sgpp::combigrid::LvlMIVec, std::vector<sgpp::combigrid::CTCoeffType> >& pairRef =
      $1;
  ...
  PyTuple_SET_ITEM(
      ...
      SWIG_NewPointerObj(new sgpp::combigrid::LvlMIVec(pairRef.first),
                         $descriptor(sgpp::combigrid::MIVec<unsigned int>*), SWIG_POINTER_OWN));
  PyTuple_SET_ITEM($result, 1, coeffs);
}

// MIVecSGGenInstr stores a *reference* to the LvlMIVec passed to its constructor, and clone()
// copies that reference (e.g. into every SparseGrid built from it). The Python LvlMIVec is
// therefore kept alive by the instruction (_miVec), by its clones (_owner, see clone below) and,
// transitively, by sparse grids built from it (_genInstr, see SparseGrid below).
% pythonappend sgpp::combigrid::MIVecSGGenInstr::MIVecSGGenInstr %
    ...
    % pythonappend sgpp::combigrid::SGGenInstr::clone % {val._owner = self % }

    % include
    "combigrid/src/sgpp/combigrid/sparse_grid_generation_instructions/sg_gen_instruction.hpp" %
    include
    "combigrid/src/sgpp/combigrid/sparse_grid_generation_instructions/"
    "complete_sg_gen_instruction.hpp" %
    include
    "combigrid/src/sgpp/combigrid/sparse_grid_generation_instructions/"
    "multiindex_vector_sg_gen_instruction.hpp"
...
% pythonappend sgpp::combigrid::TensorGrid::getGPCntPerDim % {val._owner = self % } %
  ...
  PyTuple_SET_ITEM(
      ...
      SWIG_NewPointerObj(new sgpp::combigrid::GPMI(pairRef.second),
                         $descriptor(sgpp::combigrid::MI<unsigned int>*), SWIG_POINTER_OWN));
}
%
...
% extend sgpp::combigrid::TensorGridCTData{const sgpp::combigrid::LvlMI &
                                           __getMI() const {return $self->mi;
}
const sgpp::combigrid::TensorGrid& __getTensorGrid() const { return $self->tensorGrid; }

% pythoncode % {
  @property def mi(self)
      : val=self.__getMI() val._owner=self return val

        @property def tensorGrid(self)
      : val=self.__getTensorGrid() val._owner=self return val %
}
}
% include
    "combigrid/src/sgpp/combigrid/miscellaneous/tensor_grid/"
    "tensor_grid_combination_technique_data.hpp"
...
% pythonappend sgpp::combigrid::SparseGrid::SparseGrid %
    ...
    // Note: SWIG drops the const of std::shared_ptr<const SGGenInstr>, so setters called on the
    // returned object modify the instruction the sparse grid's operators use.
    % pythonappend sgpp::combigrid::SparseGrid::getGenInstr % {val._owner = self % } %
    extend sgpp::combigrid::SparseGrid{const sgpp::combigrid::TensorGridCTData *
                                       getTensorGridByMI(const sgpp::combigrid::LvlMI& mi)
                                           const {auto it = $self->getTensorGrid(mi);
...
#'for tgData in sparseGrid' terminates.
  def __len__(self)
      ...
      : if idx < 0 or
      idx >= self.nTG()
      : raise IndexError("tensor grid index out of range") return self.getTensorGrid(idx) %
}
}
%
    ...
    // global_interpolation::interpolate and quadrature_operator::quadrature would be merged into
    // the public interpolate / quadrature functions as additional overloads.
...
COMBIGRID_SOURCE_FUNC_EXCEPTION(sgpp::combigrid::quadrature)
    ...
    % include
    "combigrid/src/sgpp/combigrid/operators/global_interpolation/global_interpolation.hpp" %
